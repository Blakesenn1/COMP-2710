...
struct Trivia {
  ...
  bool ask_question(int num_ask) {
    if (num_ask < 1) {
      ...
      cout << "Warning - there (is/are) only " << size << " trivia 
        question(s) in the list." << endl;
      return false;
    }
    else {
      score = 0;
      ...
      while (itr.has_next() && count < num_ask) {
        ...
        if (user_answer.compare(current.answer) == 0) {
          cout << "Your answer is correct. You receive " << current.value
            << " points." << endl;
          score += current.value;
        }
        else {
  ...
  void initial_questions(int version) {
    ...
    if (version > -1) { // Version 0 selects this list only
      add("How long was the shortest war on record? (Hint: how many
        minutes)", "38", 100);
    }
    if (version > 0) { // Version 1 adds this block and the previous
      add("What was Bank of America's original name? (Hint: Bank of Italy or
        Bank of Germany)?", "Bank of Germany", 50);
      add("What is the best-selling video game of all time? (Hint: Call of
        Duty or Wii Sports)?", "Wii Sports", 20);
    }
  }
};
